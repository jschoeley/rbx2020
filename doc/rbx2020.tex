...
% include exposure as it stabilizes predictions

% no wonder Euromomo underestimates baseline

% why does avg count have lower error than avg rate?

% simple robustness check is to make predictions with average death rate and
% average death count model, gives a good range of results

% no evidence that inclusion of temperature effects, autoregressive errors, or time-varying seasonality reduces error when predicting weekly or annual death counts over a single year. all regression models, as long as they controlled for population structure, were similar in terms of bias and error. this may be because a relevant factor for the number of deaths in the near term, the seasonal influenza activity, was not accounted for in any of the models we compared. In the absence of an influenza predictor the simple Serfling model and the complicated LGM models essentially do the same thing: they extrapolate long-term trends (age and sex specific rates of mortality improvement) and expect an average seasonal pattern.

% Causal inference vs. mortality monitoring

% Whether or not an influenza predictor should be included when estimating excess deaths over the COVID-19 pandemic depends on the purpose of the excess deaths analysis: causal inference or mortality monitoring. A causal estimate of the direct and indirect effects of the pandemic on overall mortality can not accomodate observed influenza activity past the beginning of the pandemic as this variable is itself influenced by the "treatment", i.e. COVID-19 and associated mitigation measures. In a counterfactual world where the COVID-19 pandemic did not happen, influenza activity throughout 2020 may have been different. For mortality monitoring purposes however, the inclusion of an influenza predictor is admissible as the question is whether the currently observed death counts are unusual given the currently observed levels of influenza (and other predictors).
